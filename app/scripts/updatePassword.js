document.addEventListener("DOMContentLoaded", async function () {
    ...
    await makeRequestWithBody(URL, bodyToken)
      .then((e) => {
        ...
        if (e.status >= 200 && e.status < 300) {
          ...
          alert("Token inválido ou expirado. Por favor, insira seu email novamente.");
          showEmailReceiverComponent(); email
        } else {
          alert(e.message); 
        }
      })
  ...
  function showEmailReceiverComponent() {
    ...
    if (container) {
      ...
      console.error("Container para o componente de email não encontrado.");
    }
  }